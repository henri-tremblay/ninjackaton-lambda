package com.ninja_squad.training.lambda.solution;

import com.ninja_squad.training.lambda.Tweet;
...
public class TPSolution {
    ...
    public static void step2() {
        Tweet.TWEETS.stream().map(Tweet::date).forEach(System.out::println);
    }
    ...
    public static List<Tweet> step6() {
        return Tweet.TWEETS.stream()
                .filter(t -> t.getHashTags().contains("#lambda"))
                .toList();
    }
    ...
    public static List<Tweet> step7() {
        return Tweet.TWEETS.stream()
                .filter(t -> t.getHashTags().contains("#lambda"))
                .sorted(Comparator.comparing(Tweet::sender).thenComparing(Tweet::date))
                .toList();
    }
    ...
    public static Map<String, List<Tweet>> step9() {
        return Tweet.TWEETS.stream()
                .collect(Collectors.groupingBy(Tweet::sender));
    }
    ...
    public static Map<Boolean, List<Tweet>> step10() {
        return Tweet.TWEETS.stream()
                .collect(Collectors.partitioningBy(t -> t.getHashTags().contains("#lambda")));
    }
    ...
    public static int step13() {
        return Tweet.TWEETS.stream()
                .map(Tweet::text)
                .mapToInt(String::length)
                .reduce(0, Integer::sum) / Tweet.TWEETS.size();
    }
